...
def solution(heights, bricks, ladders):
    ...
    for i in range(len(heights)-1):
        ...
        if len(difference) > ladders:
            bricks -= sorted(difference)[0]
            sorted(difference).pop(0)

# TODO: this does not consider if bricks never reach below 0
        if bricks < 0:
            return i   

# use print statement to check if it works
print(solution(heights, bricks, ladders))